...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        if (start == null || end == null) {
        	throw new NoSuchElementException("There is either no start or ending data");
        }
        if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
        	throw new NoSuchElementException("There is either no starting or ending vertex");
        }
        Vertex vStart = this.vertices.get(start);
        PriorityQueue<Path> pathsFound = new PriorityQueue<>();
        Hashtable <T, Vertex> visitedVertices = new Hashtable<>();
        Path copy;
        //puts the first path into the PQueue, just has one vertex
        pathsFound.add(new Path(vStart));
        visitedVertices.put(start, vStart);
        //while loop to look through the top path in the PQueue and remove it 
        //if it isn't the path that we are looking for
        while (!pathsFound.isEmpty()) {
        	Path currentPath = pathsFound.poll();
        	if (currentPath.end.data.equals(end)) {
        		return currentPath;
        	}
        	//looks through any possible new edges leading away from the last vertex
        	//from the top path in PQueue and add these new paths into the PQueue
        	for (int i = 0; i<currentPath.end.edgesLeaving.size(); i++) {
        		Edge e = currentPath.end.edgesLeaving.get(i);
        		copy = new Path(currentPath);
        		//adds the new path into the PQueue if the target vertex is not already
        		//in the HashTable of visitedVertices
        		if (!visitedVertices.containsValue(e.target)) {
	        		copy.extend(e);
	        		visitedVertices.put(e.target.data, e.target);
	        		pathsFound.add(copy);
        		}
        	}
        	
        }
        //didn't find the right path, and throws a NoSuchElementException to show that
    	throw new NoSuchElementException("We have failed to find a path between these two data points"); 
    }
